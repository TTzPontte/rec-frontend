...
class Api {
  ...
  async salvarPessoaNoProcesso(dados) {
    let processo = null;
    try {
      processo = await this.buscarProcessoByUuid(
        "/processo/".concat(dados.processo + "/0/1")
      );
    } catch (e) {
      return e.message;
    }
    ...
    if (
      StringUtils.isNotNull(pessoa) &&
      StringUtils.isNotNull(pessoa.id) &&
      StringUtils.isNotNull(processo) &&
      StringUtils.isNotNull(processo.id)
    ) {
      let processoEnvolvidos = {
        tipo: dados.envolvimento,
        pessoa: { id: pessoa.id },
        processo: { id: processo.id },
      };
      try {
        let resposta = await this.service.post(
          "/processo-envolvidos",
          processoEnvolvidos
        );
